...
const viewBase = require('tns-core-modules/ui/core/view-base');
...
function getElementById(v, id) {
        ...
        if (
            ...
            for (var key in child._realizedItems) {
                if (child._realizedItems.hasOwnProperty(key)) {
                    ...
                    if (
                        !viewCallBack(child._realizedItems[key])
                    ) {
                        return false;
                    }
                }
    ...
    if (typeof retVal === 'undefined') {
        ...
        if (
            v._realizedItems &&
            v._realizedItems.size !== v._childrenCount
        ) {
...
function getElementsByTagName(v, tagName) {
    ...
    if (
        (v.typeName && v.typeName.toLowerCase() === tagNameLC) ||
        allTags
    ) {
        retVal.push(v);
    }
    ...
    var tagNameCallback = function(child) {
        if (
            (child.typeName &&
                child.typeName.toLowerCase() === tagNameLC) ||
            allTags
        ) {
            retVal.push(child);
        }
...
var getCurrentActiveModel = function() {
    ...
    var model =
        topFrame.currentPage && topFrame.currentPage.model;
    if (model) {
        return model;
    }
    return topFrame;
...
if (!viewBase.ViewBase.prototype.getElementsByClassName) {
     ...
     * @returns {Array} of elements
     */
    viewBase.ViewBase.prototype.getElementsByClassName = function(
        className,
    ) {
...
if (!viewBase.ViewBase.prototype.getElementsByTagName) {
     ...
     * @returns {Array}
     */
    viewBase.ViewBase.prototype.getElementsByTagName = function(
        tagName,
    ) {
...
viewBase.ViewBase.prototype.runAgainstClasses = function(
    clsName,
    func,
) {
...
viewBase.ViewBase.prototype.runAgainstTagNames = function(
    tagName,
    func,
) {
